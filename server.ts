...
app.delete("/api/employees/:id", requireAppAuth, requirePermission("employees.edit"), async (req, res) => {
  ...
  app.get("/api/crm/management-dashboard", requireAppAuth, requirePermission("crm.general.view"), async (_req, res) => {
    ...
    const crmMgmtDisplayName = (companyName: string, tradeName: string | null): string => {
      const t = tradeName?.trim();
      return t && t.length > 0 ? t : companyName;
    };

    const crmMgmtDaysSince = (d: Date | null | undefined, nowMs: number): number | null => {
      if (!d) return null;
      const t = d.getTime();
    ...
    const crmMgmtIso = (d: Date | null | undefined): string | null => {
      if (!d) return null;
      const t = d.getTime();
      return Number.isFinite(t) ? d.toISOString() : null;
    };

    const activityNotClosedSql = Prisma.sql`(
      a."status" IS NULL
      OR LOWER(TRIM(a."status")) NOT IN ('done', 'closed', 'cancelled', 'canceled')
    )`;

    const proposalOpenStatusSql = Prisma.sql`p.status::text IN ('DRAFT', 'ANALYSIS', 'SENT')`;

    const proposalWithoutFollowUpNotExistsSql = Prisma.sql`
      NOT EXISTS (
        SELECT 1
        FROM "CommercialActivity" a
        WHERE a."customerId" = p."customerId"
          AND COALESCE(a."contactDate", a."createdAt") >= p."updatedAt"
          AND (a."proposalId" = p.id OR a."proposalId" IS NULL)
      )
    `;
    ...
    try {
      ...
      const validPurchaseStatusSql = Prisma.sql`so.status::text IN ('READY_TO_SEND', 'SENT_TO_NOMUS')`;
      ...
      const [
        ...
        prisma.$queryRaw<{ c: bigint }[]>(
          Prisma.sql`
            ...
            WHERE a."nextActionAt" IS NOT NULL
              AND a."nextActionAt" >= ${now}
              AND a."nextActionAt" < ${in30}
              AND ${activityNotClosedSql}
          `
        ),
        prisma.$queryRaw<{ cnt: bigint; val: unknown }[]>(
          Prisma.sql`
            SELECT COUNT(*)::bigint AS cnt, COALESCE(SUM(p."totalNetValue"), 0) AS val
            FROM "Proposal" p
        ...
        prisma.$queryRaw<{ c: bigint }[]>(
          Prisma.sql`
            SELECT COUNT(*)::bigint AS c
            FROM "Proposal" p
            WHERE ${proposalOpenStatusSql}
              AND ${proposalWithoutFollowUpNotExistsSql}
          `
        ),
        prisma.$queryRaw<{ c: bigint }[]>(
          Prisma.sql`
        ...
        >(
          Prisma.sql`
            ...
            open_proposal_stats AS (
              SELECT p."customerId",
                COUNT(*)::int AS cnt,
                COALESCE(SUM(p."totalNetValue"), 0) AS val
              FROM "Proposal" p
              WHERE p.status::text IN ('DRAFT', 'ANALYSIS', 'SENT')
            ...
            scored AS (
              SELECT
                ...
                COALESCE(ops.cnt, 0) AS open_proposals_count,
                COALESCE(ops.val, 0) AS open_proposals_value,
                (cnf."customerId" IS NOT NULL) AS has_prop_no_fu,
                ...
                CASE
                  ...
                  THEN 'HIGH'
                  WHEN lp.last_at IS NULL OR COALESCE(ops.cnt, 0) > 0
                  THEN 'MEDIUM'
                  ELSE 'LOW'
                END AS risk_level
              FROM "Customer" c
              ...
              LEFT JOIN open_proposal_stats ops ON ops."customerId" = c."id"
            ...
            ORDER BY
              ...
              has_prop_no_fu DESC,
              CASE
                WHEN last_purchase_at IS NULL THEN 999999
                ELSE EXTRACT(EPOCH FROM (${now}::timestamptz - last_purchase_at)) / 86400
              END DESC,
              CASE
                WHEN last_contact_at IS NULL THEN 999999
                ELSE EXTRACT(EPOCH FROM (${now}::timestamptz - last_contact_at)) / 86400
              END DESC
            LIMIT ${CRM_MGMT_LIST_LIMIT}
          `
        ),
        prisma.$queryRaw<
          {
        ...
        >(
          Prisma.sql`
            ...
            purchase_12m AS (
              ...
              FROM "SalesOrder" so
              WHERE ${validPurchaseStatusSql}
                AND so."issueDate" >= ${twelveMonthsAgo}
              GROUP BY so."customerId"
            ),
            open_proposal_stats AS (
              SELECT p."customerId", COUNT(*)::int AS cnt
              FROM "Proposal" p
              WHERE p.status::text IN ('DRAFT', 'ANALYSIS', 'SENT')
              GROUP BY p."customerId"
            )
            SELECT
              ...
              lp.last_at AS last_purchase_at,
              lc.last_at AS last_contact_at,
              COALESCE(p12.total_12m, 0) AS total_12m,
              COALESCE(ops.cnt, 0) AS open_proposals_count
            FROM "Customer" c
        ...
        >(
          Prisma.sql`
            ...
            SELECT
              ...
              COALESCE(ops.cnt, 0) AS open_proposals_count
            FROM "Customer" c
            INNER JOIN open_proposal_stats ops ON ops."customerId" = c."id"
            LEFT JOIN last_purchase lp ON lp."customerId" = c."id"
            LEFT JOIN last_contact lc ON lc."customerId" = c."id"
            LEFT JOIN purchase_12m p12 ON p12."customerId" = c."id"
            LEFT JOIN customer_no_followup cnf ON cnf."customerId" = c."id"
            WHERE cnf."customerId" IS NULL
              AND ops.cnt > 0
            ORDER BY COALESCE(p12.total_12m, 0) DESC, ops.cnt DESC
            LIMIT ${CRM_MGMT_LIST_LIMIT}
          `
        ),
        prisma.$queryRaw<
          ...
          Prisma.sql`
            ...
            purchase_12m AS (
              SELECT so."customerId",
                COALESCE(SUM(so."totalNetValue"), 0) AS total_12m
              FROM "SalesOrder" so
              WHERE ${validPurchaseStatusSql}
                AND so."issueDate" >= ${twelveMonthsAgo}
              GROUP BY so."customerId"
        ...
        >(
          Prisma.sql`
            SELECT
              ...
              c."companyName" AS company_name,
              c."tradeName" AS trade_name,
              a."nextActionAt" AS next_action_at,
              a."nextActionDescription" AS next_action_description,
              a."assignedTo" AS assigned_to,
              a."createdByName" AS created_by_name
            FROM "CommercialActivity" a
        ...
        prisma.$queryRaw<{ key: string | null; count: bigint }[]>(
          Prisma.sql`
            SELECT COALESCE(NULLIF(TRIM(a.channel), ''), 'Sem canal') AS key,
        ...
        prisma.$queryRaw<{ key: string | null; count: bigint }[]>(
          Prisma.sql`
            SELECT COALESCE(NULLIF(TRIM(a.reason), ''), 'Sem motivo') AS key,
              COUNT(*)::bigint AS count
            FROM "CommercialActivity" a
            WHERE COALESCE(a."contactDate", a."createdAt") >= ${since30}
            GROUP BY 1
      ...
      const bigintCount = (rows: { c: bigint }[] | undefined) =>
        Number(rows?.[0]?.c ?? 0n);

      const openProposalsAgg = openProposalsRow?.[0];
      ...
      const buildRiskReasons = (
        ...
        if (hasPropNoFu) reasons.push("PROPOSAL_WITHOUT_FOLLOW_UP");
        if (lastPurchaseAt && lastPurchaseAt < since90) {
          reasons.push("NO_PURCHASE_90D");
        } else if (!lastPurchaseAt) {
          reasons.push("NO_VALID_PURCHASE");
        }
        if (openProposalsCount > 0 && riskLevel === "MEDIUM") {
          reasons.push("OPEN_PROPOSALS");
        }
        return reasons;
      };
      ...
      const riskCustomers = riskCustomersRows.map((row) => {
        ...
        const daysSinceLastPurchase = crmMgmtDaysSince(row.last_purchase_at, nowMs);
        const daysSinceLastContact = crmMgmtDaysSince(row.last_contact_at, nowMs);
        return {
          customerId: row.customer_id,
          displayName: crmMgmtDisplayName(row.company_name, row.trade_name),
          taxId: row.tax_id,
          city: row.city ?? null,
          ...
          riskReasons: buildRiskReasons(
            ...
            row.open_proposals_count
          ),
          daysSinceLastPurchase,
          daysSinceLastContact,
          openProposalsCount: row.open_proposals_count,
          openProposalsValue: crmMgmtToNumber(row.open_proposals_value),
          nextFollowUpAt: crmMgmtIso(row.next_followup_at),
      ...
      const buildSuggestedAction = (
        ...
        if (tier === 1 || (lastPurchaseAt && lastPurchaseAt >= since30)) {
          return "Realizar pós-venda e identificar nova oportunidade.";
        }
        if (openProposalsCount > 0) {
          return "Acompanhar proposta aberta com follow-up em dia.";
        }
        if (!lastContactAt || lastContactAt < since30) {
          return "Retomar contato com cliente de alto valor.";
        }
        return "Manter acompanhamento comercial.";
      };
      ...
      const opportunityMap = new Map<string, OpportunityCandidate>();

      const addOpportunityRows = (
        rows: typeof opportunityTier1Rows,
        tier: number
      ) => {
        for (const row of rows) {
          ...
          opportunityMap.set(row.customer_id, {
            ...
            displayName: crmMgmtDisplayName(row.company_name, row.trade_name),
            taxId: row.tax_id,
            daysSinceLastPurchase: crmMgmtDaysSince(row.last_purchase_at, nowMs),
            daysSinceLastContact: crmMgmtDaysSince(row.last_contact_at, nowMs),
            totalPurchasedLast12Months: crmMgmtToNumber(row.total_12m),
            openProposalsCount: row.open_proposals_count,
      ...
      const overdueFollowUps = overdueFollowUpListRows.map((row) => {
        ...
        const daysOverdue = Math.max(
          0,
          Math.floor((nowMs - nextAt.getTime()) / 86400000)
        );
        return {
          activityId: row.activity_id,
          customerId: row.customer_id,
          displayName: crmMgmtDisplayName(row.company_name, row.trade_name),
          nextActionAt: crmMgmtIso(nextAt) ?? now.toISOString(),
      ...
      const upcomingFollowUps = upcomingFollowUpListRows.map((row) => {
        ...
        const daysUntil = Math.max(
          0,
          Math.ceil((nextAt.getTime() - nowMs) / 86400000)
        );
        return {
          activityId: row.activity_id,
          customerId: row.customer_id,
      ...
      const openProposalsWithoutFollowUp = openProposalsNoFollowUpRows.map((row) => {
        const updatedAt = row.updated_at;
        const daysWithoutFollowUp = Math.max(
          0,
          Math.floor((nowMs - updatedAt.getTime()) / 86400000)
        );
        return {
          ...
          displayName: crmMgmtDisplayName(row.company_name, row.trade_name),
          status: row.status,
          totalNetValue: crmMgmtToNumber(row.total_net_value),
          updatedAt: crmMgmtIso(updatedAt) ?? now.toISOString(),
          daysWithoutFollowUp,
      ...
      const topCustomersLast12Months = topCustomers12mRows.map((row) => ({
        ...
        displayName: crmMgmtDisplayName(row.company_name, row.trade_name),
        taxId: row.tax_id,
        totalPurchasedLast12Months: crmMgmtToNumber(row.total_purchased),
        ordersCount: Number(row.orders_count ?? 0n),
        lastPurchaseAt: crmMgmtIso(row.last_purchase_at),
        daysSinceLastContact: crmMgmtDaysSince(row.last_contact_at, nowMs),
      }));
      ...
      res.json({
        ...
        summary: {
          ...
          openProposalsCount: Number(openProposalsAgg?.cnt ?? 0n),
          openProposalsValue: crmMgmtToNumber(openProposalsAgg?.val),
          proposalsWithoutFollowUpCount: bigintCount(proposalsNoFollowUpRow),
