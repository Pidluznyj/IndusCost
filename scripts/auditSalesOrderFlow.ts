/**
 * OP-78 — Auditoria read-only do ciclo de vida do Fluxo de Pedidos.
 *
 * Uso:
...
import {
  ...
  SALES_ORDER_FLOW_PRODUCTION_AUDIT_LOG_PREFIX,
  stringifySalesOrderFlowProductionAuditReport,
} from "../src/lib/sales/salesOrderFlowProductionAudit.js";
import { loadSalesOrderFlowProductionAudit } from "../src/lib/sales/salesOrderFlowProductionAudit.server.js";

const LOG = SALES_ORDER_FLOW_PRODUCTION_AUDIT_LOG_PREFIX;
...
function defaultOutputPaths(orderCode: string): {
  ...
  return {
    jsonOutput: resolve(
      `docs/generated/sales-order-flow-audit-${safe}.json`
    ),
    markdownOutput: resolve(
      `docs/generated/sales-order-flow-audit-${safe}.md`
    ),
  };
}
...
async function main(): Promise<void> {
  let args;
  try {
    args = parseSalesOrderFlowProductionAuditArgs(process.argv.slice(2));
  } catch (error) {
    console.error(
    ...
    process.exitCode = resolveSalesOrderFlowProductionAuditExitCode(
      "technical_error"
    );
    return;
  }
  ...
  try {
    const report = await loadSalesOrderFlowProductionAudit(prisma, args.order);
    const json = stringifySalesOrderFlowProductionAuditReport(report);
    const markdown = formatSalesOrderFlowProductionAuditMarkdown(report);
    ...
    console.warn(
      `${LOG} concluído: status=${report.status} itens=${report.items.length} ` +
        `divergencia=${report.divergence.hasDivergence} ` +
        `inconsistencias=${report.inconsistencies.length}`
    );
    process.exitCode = resolveSalesOrderFlowProductionAuditExitCode("ok");
  } catch (error) {
    console.error(
      `${LOG} falha técnica: ${
        error instanceof Error ? error.message : String(error)
