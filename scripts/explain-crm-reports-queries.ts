 ...
 * consulta, mas são só SELECTs — e a conexão é aberta em sessão
 * `default_transaction_read_only` (qualquer escrita seria recusada pelo banco).
 *
...
async function main(): Promise<void> {
  ...
  let capturing = false;
  const { prisma } = await openReadOnlyAuditPrisma({
    onQuery: (event) => {
      if (!capturing) return;
      let params: unknown[] = [];
      try {
  ...
  try {
    ...
    console.log("Sessão do banco: somente leitura (default_transaction_read_only = on).");
