...
export function buildMaterialPlannedRealizedDetails(
  ...
  const products = new Map<
    string,
    { productId: string; productSku: string | null; productName: string | null; plannedQuantity: number; realizedQuantity: number }
  >();
  const plannedOrders = new Map<string, import("./materialDemandPlannedRealizedTypes.js").MaterialPlannedRealizedDetailOrder>();
  const realizedOrders = new Map<string, import("./materialDemandPlannedRealizedTypes.js").MaterialPlannedRealizedDetailOrder>();
  ...
  for (const c of filtered) {
    ...
    prod.plannedQuantity += plannedQty;
    prod.realizedQuantity += realizedQty;
    products.set(c.productId, prod);
    ...
    const plannedOrd =
      plannedOrders.get(c.orderId) ??
      { ...baseOrder, plannedQuantity: 0, realizedQuantity: 0 };
    plannedOrd.plannedQuantity += plannedQty;
    plannedOrders.set(c.orderId, plannedOrd);

    if (realizedQty > 0) {
      const realizedOrd =
        realizedOrders.get(c.orderId) ??
        { ...baseOrder, plannedQuantity: 0, realizedQuantity: 0 };
      realizedOrd.realizedQuantity += realizedQty;
      realizedOrders.set(c.orderId, realizedOrd);
    }
  }
  ...
  return {
    ...
    materialName: sample.materialDescription,
    unitLabel: sample.unitLabel,
    products: [...products.values()].sort((a, b) => b.plannedQuantity - a.plannedQuantity),
    plannedOrders: [...plannedOrders.values()].sort((a, b) => b.plannedQuantity - a.plannedQuantity),
    realizedOrders: [...realizedOrders.values()].sort((a, b) => b.realizedQuantity - a.realizedQuantity),
  };
}
