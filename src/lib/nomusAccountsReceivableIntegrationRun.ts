...
import { NOMUS_AR_SYNC_TARGET } from "./nomusAccountsReceivableSyncConstants.js";
...
export type AccountsReceivableIntegrationRunInput = {
  mode: "apply" | "preview";
  startedAt: Date;
  finishedAt: Date;
  durationMs: number;
...
export async function persistAccountsReceivableIntegrationRun(
  input: AccountsReceivableIntegrationRunInput
): Promise<void> {
  ...
  const integrationRunData: Prisma.IntegrationRunUncheckedCreateInput = {
    sourceSystem: "NOMUS",
    kind: "sync",
    target: NOMUS_AR_SYNC_TARGET,
    mode: input.mode === "apply" ? "apply" : "dry",
    status,
    success,
    command: input.command ?? "sync:nomus:accounts-receivable:apply",
  ...
  try {
    ...
    console.error("[nomus-accounts-receivable] falha ao registrar IntegrationRun:", err);
  }
}

export async function disconnectAccountsReceivableIntegrationPrisma(): Promise<void> {
