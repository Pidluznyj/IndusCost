...
import {
  ...
  resolveMaterialMarketAlertConfigValues,
  serializeMaterialMarketAlertConfigForApi,
  summarizeMaterialMarketAlertConfigChange,
  ...
  app.put(
    ...
    async (req, res) => {
      try {
        ...
        const parsed = parseMaterialMarketAlertConfigPatch(req.body ?? {});
        if (parsed.ok === false) {
          return res.status(400).json({
        ...
        const globalConfig = await prisma.materialMarketAlertGlobalConfig.findUnique({
          where: { id: "GLOBAL" },
        });
        const existingOverride = await prisma.materialMarketAlertConfig.findUnique({
          where: { materialId },
        });

        const beforeEffective = resolveMaterialMarketAlertConfigValues(
          existingOverride ?? undefined,
          globalConfig ?? undefined
        );
        const afterEffective = resolveMaterialMarketAlertConfigValues(
          { ...beforeEffective, ...parsed.value },
          globalConfig ?? undefined
        );

        const auditValidation = buildMaterialMarketAuditEventData({
          materialId,
          ...
          metadata: {
            summary: summarizeMaterialMarketAlertConfigChange({
              before: beforeEffective,
              after: afterEffective,
            }),
          },
        });
        if (auditValidation.ok === false) {
          return res.status(400).json({
            error: auditValidation.code,
            field: auditValidation.field,
            message: auditValidation.message,
          });
        }

        const saved = await prisma.$transaction(async (tx) => {
          const config = await tx.materialMarketAlertConfig.upsert({
            where: { materialId },
            create: { materialId, ...parsed.value, updatedBy: authUser.id },
            update: { ...parsed.value, updatedBy: authUser.id },
          });

          await tx.materialMarketAlertConfigAudit.create({
            data: {
              scope: "MATERIAL",
              materialId,
              beforeJson: beforeEffective,
          ...
          await recordMaterialMarketAuditEvent(tx, {
            ...
            beforeJson: beforeEffective,
            afterJson: afterEffective,
            metadata: auditValidation.data.metadata,
          });
        ...
        return res.json(
          serializeMaterialMarketAlertConfigForApi(
            resolveMaterialMarketAlertConfigValues(saved, globalConfig ?? undefined),
            { scope: "MATERIAL", materialId, hasOverride: true }
          )
        );
      } catch (error) {
        console.error("PUT material alert config", error);
        return res.status(500).json({
          error: error instanceof Error ? error.message : "Erro ao salvar configuração de alertas.",
        });
      }
    }
  );
