export type NomusArOverallStatus =
  | "RUNNING"
  | "SUCCESS"
  | "FAILED"
  | "STALE"
...
export type NomusAccountsReceivableSyncStatusPayload = {
  overallStatus: NomusArOverallStatus;
  isRunning: boolean;
...
export function arOverallStatusLabel(status: NomusArOverallStatus): string {
  switch (status) {
    case "RUNNING":
      return "Em execução";
    case "SUCCESS":
...
export function arOverallStatusBadgeClass(status: NomusArOverallStatus): string {
  switch (status) {
    case "RUNNING":
...
export function arPrimaryButtonLabel(
  overall: NomusArOverallStatus,
  isRunning: boolean
): string {
  if (isRunning || overall === "RUNNING") return "Sincronizando…";
  return "Rodar Contas a Receber agora";
}
