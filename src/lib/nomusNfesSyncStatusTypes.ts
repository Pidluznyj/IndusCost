export type NomusNfeOverallStatus =
  | "RUNNING"
  | "SUCCESS"
  | "FAILED"
  | "STALE"
...
export type NomusNfesSyncStatusPayload = {
  overallStatus: NomusNfeOverallStatus;
  isRunning: boolean;
...
export function nfeOverallStatusLabel(status: NomusNfeOverallStatus): string {
  switch (status) {
    case "RUNNING":
      return "Em execução";
    case "SUCCESS":
...
export function nfeOverallStatusBadgeClass(status: NomusNfeOverallStatus): string {
  switch (status) {
    case "RUNNING":
...
export function nfePrimaryButtonLabel(overall: NomusNfeOverallStatus, isRunning: boolean): string {
  if (isRunning || overall === "RUNNING") return "Sincronizando…";
  return "Atualizar faturamento";
}
