import {
  NOMUS_NFE_LOG_STALE_RUNNING_MS,
  NOMUS_NFE_RUNNER_LOG_RE,
} from "./nomusNfesSyncConstants.js";

export type ParsedNfeRunnerLog = {
...
export function isNfeRunnerLogFileName(fileName: string): boolean {
  return NOMUS_NFE_RUNNER_LOG_RE.test(fileName);
}

export function parseNfeRunnerLogContent(content: string): ParsedNfeRunnerLog {
...
export function computeNfeOverallStatus(input: {
  hasLiveProcess: boolean;
  hasActiveLock: boolean;
  parsed: ParsedNfeRunnerLog;
  ...
  if (parsed.status === "running") {
    const staleByAge = logAgeMs != null && logAgeMs > NOMUS_NFE_LOG_STALE_RUNNING_MS;
  ...
  if (parsed.status === "success") {
    if (logAgeMs != null && logAgeMs > NOMUS_NFE_LOG_STALE_RUNNING_MS * 4) {
      return {
...
export function buildNfeRecommendedAction(input: {
...
export function parseNfeDurationMs(startedAt: string | null, finishedAt: string | null): number | null {
