import {
  NOMUS_AP_LOG_STALE_RUNNING_MS,
  NOMUS_AP_RUNNER_LOG_RE,
} from "./nomusAccountsPayableSyncConstants.js";

export type ParsedApRunnerLog = {
...
export function isApRunnerLogFileName(fileName: string): boolean {
  return NOMUS_AP_RUNNER_LOG_RE.test(fileName);
}

export function parseApRunnerLogContent(content: string): ParsedApRunnerLog {
...
export function computeApOverallStatus(input: {
  hasLiveProcess: boolean;
  hasActiveLock: boolean;
  parsed: ParsedApRunnerLog;
  ...
  if (parsed.status === "running") {
    const staleByAge =
      logAgeMs != null && logAgeMs > NOMUS_AP_LOG_STALE_RUNNING_MS;
  ...
  if (parsed.status === "success") {
    if (logAgeMs != null && logAgeMs > NOMUS_AP_LOG_STALE_RUNNING_MS * 4) {
      return {
...
export function buildApRecommendedAction(input: {
...
export function parseApDurationMs(startedAt: string | null, finishedAt: string | null): number | null {
