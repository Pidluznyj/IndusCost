...
export function parsePortfolioIntelligenceFilters(
  ...
  return {
    ...
    statusPrincipal,
    financialStatus: asQueryString(query.financialStatus)?.toUpperCase() ?? null,
    operationalStatus: asQueryString(query.operationalStatus)?.toUpperCase() ?? null,
    confidenceLabel,
    tagsAlerta: parseTagsAlerta(query.tagsAlerta),
    operationalAlert: asQueryString(query.operationalAlert)?.toUpperCase() ?? null,
