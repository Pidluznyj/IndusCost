...
import { NOMUS_AP_SYNC_TARGET } from "./nomusAccountsPayableSyncConstants.js";
...
export type AccountsPayableIntegrationRunInput = {
  mode: "apply" | "preview";
  startedAt: Date;
  finishedAt: Date;
  durationMs: number;
...
export async function persistAccountsPayableIntegrationRun(
  input: AccountsPayableIntegrationRunInput
): Promise<void> {
  ...
  const integrationRunData: Prisma.IntegrationRunUncheckedCreateInput = {
    sourceSystem: "NOMUS",
    kind: "sync",
    target: NOMUS_AP_SYNC_TARGET,
    mode: input.mode === "apply" ? "apply" : "dry",
    status,
    success,
    command: input.command ?? "sync:nomus:accounts-payable:apply",
  ...
  try {
    ...
    console.error("[nomus-accounts-payable] falha ao registrar IntegrationRun:", err);
  }
}

export async function disconnectAccountsPayableIntegrationPrisma(): Promise<void> {
