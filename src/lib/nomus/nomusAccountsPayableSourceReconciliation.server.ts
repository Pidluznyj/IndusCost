/**
 * SYNC-06 — Persistência de lifecycle de Contas a Pagar + lock.
...
import { isNomusAccountsPayableAbsenceReconciliationEnabled } from "./nomusSourceReconciliationFlags.js";
import type { NomusSourceLifecyclePatch } from "./nomusSourceReconciliationEngine.js";
import type { AccountsPayableLifecycleLocalSnapshot } from "./nomusAccountsPayableSourceReconciliation.js";
import {
  buildNomusUrl,
  fetchNomusJson,
...
import { toInt } from "../nomusAccountsPayableParser.js";

export const NOMUS_AP_RECONCILE_LOCK_ENV =
  "NOMUS_ACCOUNTS_PAYABLE_RECONCILE_LOCK_FILE";
export const NOMUS_AP_RECONCILE_LOCK_DEFAULT =
  "/tmp/induscost-nomus-accounts-payable-reconcile.lock";
...
export function resolveAccountsPayableReconcileLockFile(
  env: NodeJS.ProcessEnv = process.env
): string {
  const raw = (env[NOMUS_AP_RECONCILE_LOCK_ENV] ?? "").trim();
  return raw || NOMUS_AP_RECONCILE_LOCK_DEFAULT;
}

export function acquireAccountsPayableReconcileLock(input: {
  mode: "preview" | "apply";
  lockFile?: string;
  ...
  const lockFile =
    input.lockFile ?? resolveAccountsPayableReconcileLockFile(input.env);
    ...
    try {
      ...
      if (
        ...
        return {
          ok: false,
          code: "LOCK_HELD",
          message: `Reconciliação de Contas a Pagar já em andamento (pid=${parsed.pid}).`,
          lockFile,
        };
      }
      unlinkSync(lockFile);
...
export async function loadAccountsPayableLifecycleLocals(input: {
  prisma: PrismaClient;
  dueDateFrom?: Date | null;
  dueDateTo?: Date | null;
  externalIds?: number[];
}): Promise<AccountsPayableLifecycleLocalSnapshot[]> {
  ...
  const rows = await input.prisma.nomusAccountsPayable.findMany({
    where,
    select: {
      id: true,
      externalId: true,
  ...
  return rows.map((r) => ({
    ...
    balancePayable: decimalToNumber(r.balancePayable),
...
export async function createAccountsPayableSourceSyncRun(input: {
  prisma: PrismaClient;
  strategy: string;
...
export async function finishAccountsPayableSourceSyncRun(input: {
  prisma: PrismaClient;
  runId: string;
...
): Prisma.NomusAccountsPayableUpdateInput {
  const data: Prisma.NomusAccountsPayableUpdateInput = {
...
export async function applyAccountsPayableLifecyclePatches(input: {
  prisma: PrismaClient;
...
export function isAccountsPayableAbsenceReconcileEnabled(
  env: NodeJS.ProcessEnv = process.env
): boolean {
 ...
 * (não há GET /contasPagar/{id} no IndusCost).
 */
export async function lookupNomusAccountsPayableByExternalId(args: {
  baseUrl: string;
  externalId: number;
  maxPages?: number;
