/**
 * SYNC-05 — Adapter puro: Contas a Receber ↔ motor SYNC-03.
 *
 * Independência dura: ausência de Pedido NÃO implica ausência de CR
 * (ex.: PD 02739 vs CR externalId 17748).
 *
 * Sem HTTP/Prisma.
 */
...
export type AccountsReceivableAuthoritativeScopeKind =
  | "DUE_DATE_WINDOW_ALL_TITLES"
  | "OPEN_RECEIVABLES_SCOPE";

export type AccountsReceivableFetchCompletenessAssessment = {
  payloadComplete: boolean;
  status: "COMPLETE" | "INCONCLUSIVE_FETCH" | "OPEN_SCOPE_COMPLETE";
  /** Label operacional do syncer (não prova completude sozinho). */
  syncStrategy: string;
  authoritativeScope: AccountsReceivableAuthoritativeScopeKind;
  onlyPending: boolean;
  reasons: string[];
  startPage: number;
...
export type AccountsReceivableFetchMetaForCompleteness = {
  syncStrategy: string;
  startPage: number;
  maxPages: number;
  pagesRead: number;
 ...
 */
export function assessAccountsReceivableSyncPayloadCompleteness(
  meta: AccountsReceivableFetchMetaForCompleteness
): AccountsReceivableFetchCompletenessAssessment {
  const http429Count = meta.http429Count ?? 0;
  const errors = [...(meta.errors ?? [])];
  const reasons: string[] = [];
  const authoritativeScope: AccountsReceivableAuthoritativeScopeKind =
    meta.onlyPending
      ? "OPEN_RECEIVABLES_SCOPE"
      : "DUE_DATE_WINDOW_ALL_TITLES";

  if (meta.onlyPending) {
    reasons.push("OPEN_RECEIVABLES_SCOPE");
...
export function buildAccountsReceivableSyncReconciliationScope(input: {
  from: string;
  to: string;
  onlyPending: boolean;
  syncStrategy: string;
}): NomusSourceSyncScope {
  return buildAccountsReceivableDueDateScope({
    from: input.from,
    to: input.to,
    onlyPending: input.onlyPending,
    strategy: input.onlyPending
      ? `OPEN_RECEIVABLES_SCOPE:${input.syncStrategy}`
      : input.syncStrategy,
  });
}

/** Título ainda no universo aberto (não liquidado / com saldo). */
export function isOpenAccountsReceivableTitle(input: {
  ...
  const bal = input.balanceReceivable ?? 0;
  return Number.isFinite(bal) && bal > 0.000001;
}

export type AccountsReceivableLifecycleLocalSnapshot = {
  localId: string;
  externalId: number;
...
export function toAccountsReceivableLocalRecord(
  row: AccountsReceivableLifecycleLocalSnapshot,
...
/**
 * Em OPEN_RECEIVABLES_SCOPE, só títulos abertos entram na avaliação de ausência.
 * Pagos históricos ficam fora (preservados).
 */
export function selectLocalsForAccountsReceivableAbsenceUniverse(input: {
  locals: readonly AccountsReceivableLifecycleLocalSnapshot[];
  onlyPending: boolean;
}): {
  inUniverse: AccountsReceivableLifecycleLocalSnapshot[];
  preservedHistoricalPaid: AccountsReceivableLifecycleLocalSnapshot[];
} {
  if (!input.onlyPending) {
    return { inUniverse: [...input.locals], preservedHistoricalPaid: [] };
  }
  const inUniverse: AccountsReceivableLifecycleLocalSnapshot[] = [];
  const preservedHistoricalPaid: AccountsReceivableLifecycleLocalSnapshot[] = [];
...
export type BuildAccountsReceivableReconciliationPlanArgs = {
  syncStrategy: string;
  scope: NomusSourceSyncScope;
  completeness: AccountsReceivableFetchCompletenessAssessment;
  reconciliationEnabled: boolean;
  foundRows: ReadonlyArray<{ externalId: number; payloadHash: string }>;
  localRecords: readonly AccountsReceivableLifecycleLocalSnapshot[];
...
export function buildAccountsReceivableSourceReconciliationPlan(
  args: BuildAccountsReceivableReconciliationPlanArgs
  ...
  const localRecords = inUniverse.map((row) =>
    toAccountsReceivableLocalRecord(row, args.scope)
  );
  ...
  for (const paid of preservedHistoricalPaid) {
    plan.ignoredOutsideScope.push({
      ...
      entityType: "ACCOUNTS_RECEIVABLE",
      reason: "PAID_HISTORICAL_OUTSIDE_OPEN_RECEIVABLES_SCOPE",
  ...
  plan.reasons = [
    ...new Set([
      ...
      "SALES_ORDER_ABSENCE_DOES_NOT_IMPLY_AR_ABSENCE",
    ]),
  ];
...
export function planDirectedAccountsReceivableAbsenceConfirmation(input: {
  local: AccountsReceivableLifecycleLocalSnapshot;
  scope: NomusSourceSyncScope;
...
export function buildPresentAccountsReceivableLifecycleWriteData(input: {
  payloadHash: string;
  executedAt: Date;
  runId: string | null;
  isCreate: boolean;
...
export type AccountsReceivableReconciliationPreviewSummary = {
  ...
  fetchCompleteness: AccountsReceivableFetchCompletenessAssessment;
  scope: NomusSourceSyncScope;
  authoritativeScope: AccountsReceivableAuthoritativeScopeKind;
  ...
  totalReceivedHistoricalProtected: number;
...
export function summarizeAccountsReceivableReconciliationPreview(
  plan: NomusSourceReconciliationPlan,
  completeness: AccountsReceivableFetchCompletenessAssessment,
  scope: NomusSourceSyncScope,
  localsByExternalId?: ReadonlyMap<string, AccountsReceivableLifecycleLocalSnapshot>
): AccountsReceivableReconciliationPreviewSummary {
  const enrichMissing = (items: NomusSourceReconciliationItem[]) =>
    items.map((i) => {
      ...
      return {
        ...
        localId: i.localId,
        reason: i.reason,
        balanceReceivable: local?.balanceReceivable ?? null,
        amountReceived: local?.amountReceived ?? null,
      };
    });
  ...
  let totalReceivedHistoricalProtected = 0;
  for (const item of missingAll) {
    const local = localsByExternalId?.get(item.externalId);
    totalOpenAffected += moneyOf(local?.balanceReceivable);
    totalReceivedHistoricalProtected += moneyOf(local?.amountReceived);
  }
...
export const ACCOUNTS_RECEIVABLE_PILOT = {
  externalId: 17748,
  relatedOrderCode: "PD 02739",
  relatedExternalSalesOrderId: 2737,
} as const;

export function isAccountsReceivablePilot(externalId: number | null | undefined): boolean {
  return externalId === ACCOUNTS_RECEIVABLE_PILOT.externalId;
}
