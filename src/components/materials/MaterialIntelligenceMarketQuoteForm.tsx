...
import { MaterialMarketQuoteSupplierField } from "@/src/components/materials/MaterialMarketQuoteSupplierField";
...
export function MaterialIntelligenceMarketQuoteForm({
  ...
  }, [canManualExchange, form.quoteDate, isUsd]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!form.supplierName.trim()) {
      setError("Informe o fornecedor (cadastrado ou nome livre).");
      return;
    }
  ...
  return (
    <form
      ...
      className="space-y-4"
      data-testid="material-intelligence-market-quote-form"
    >
      <div className="grid gap-3 sm:grid-cols-2">
        <div className="space-y-1 text-sm sm:col-span-2">
          <span className="font-medium">Fornecedor *</span>
          <MaterialMarketQuoteSupplierField
            value={{ supplierId: form.supplierId, supplierName: form.supplierName }}
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        {onCancel ? (
          <button
            ...
            onClick={onCancel}
            disabled={submitting}
            className="rounded-lg border border-border px-4 py-2 text-sm font-semibold hover:bg-muted/50 disabled:opacity-60"
            data-testid="material-market-quote-cancel"
          >
            Cancelar
          </button>
        ) : null}
