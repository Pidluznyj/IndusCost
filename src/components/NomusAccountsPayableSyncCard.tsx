...
export function NomusAccountsPayableSyncCard({
  ...
  const loadStatus = useCallback(async () => {
    ...
    try {
      ...
      setError(
        e instanceof Error
          ? e.message
          : "Não foi possível carregar o status de Contas a Pagar Nomus."
      );
      setStatus(null);
    } finally {
      setLoading(false);
  ...
  const handleConfirmRun = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(body.error ?? "Não foi possível iniciar a sincronização de Contas a Pagar.");
      }
      setRunMessage(
        body.message ??
          "Sincronização de Contas a Pagar iniciada. Acompanhe o status e os logs abaixo."
      );
      setModalOpen(false);
      setConfirmText("");
      void loadStatus();
  ...
  const buttonLabel = apPrimaryButtonLabel(overall, isActuallyRunning);
      ...
      <div className="rounded-xl border border-border bg-card/60 p-5 space-y-4">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div className="min-w-0 space-y-1">
            <h4 className="text-base font-bold text-foreground">Contas a Pagar Nomus</h4>
            <p className="text-sm text-muted-foreground">
              Rotina isolada de contas a pagar (modo apply). Agendamento automático a cada 2 horas;
          ...
          <div className="flex flex-wrap gap-2 shrink-0">
            ...
            {canRun ? (
              <button
                ...
                title="Executa manualmente somente a sincronização de Contas a Pagar Nomus."
      ...
      {modalOpen ? (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-black/50">
          <div
            ...
            role="dialog"
            aria-modal="true"
            aria-labelledby="nomus-ap-sync-title"
          >
            <div>
              <h3 id="nomus-ap-sync-title" className="text-lg font-bold">
                Confirmar Contas a Pagar Nomus
              </h3>
            ...
            <div className="space-y-2">
              <label
                htmlFor="nomus-ap-confirm"
                className="text-xs font-bold uppercase text-muted-foreground"
              >
                Digite para confirmar: {NOMUS_AP_SYNC_CONFIRM_PHRASE}
              </label>
              <input
                id="nomus-ap-confirm"
                value={confirmText}
