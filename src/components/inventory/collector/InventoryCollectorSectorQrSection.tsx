...
// CSS de impressão importado pelo container (InventoryCollectorDevicesTab), não
// aqui — mantém este componente puramente apresentacional e testável sob
// `node --test` (sem bundler, que não resolve import estático de .css).
...
/** Exportado à parte: puro, sem `document`, testável diretamente sem portal. */
export function CollectorSectorQrPrintSheet({ data }: { data: CollectorSectorQrResponse }) {
  return (
...
export function InventoryCollectorSectorQrSection({
  ...
  return (
    <section className="space-y-3" data-testid="collector-sector-qr-section">
      {state.status === "ready" && typeof document !== "undefined"
