...
export function FinanceAccountsReceivableOverduePrintDocument({
  ...
  return (
    <div id="ar-overdue-print-root" className="finance-ar-overdue-print-document">
      ...
      <section className="finance-ar-overdue-print-section">
        ...
        <table className="finance-ar-overdue-print-data-table">
          <thead>
            <tr>
              <th>Faixa</th>
              <th className="col-num">Qtde. títulos</th>
        ...
        <table className="finance-ar-overdue-print-data-table">
          <thead>
            <tr>
              ...
              <th>Cliente</th>
              <th>CNPJ/CPF</th>
              <th className="col-num">Qtd. títulos</th>
              <th className="col-money">Valor vencido</th>
      ...
      <section className="finance-ar-overdue-print-section finance-ar-overdue-print-detail-section">
          ...
          customerGroups.map((group) => (
            <div key={group.customerKey} className="finance-ar-overdue-print-customer-group">
              <div className="finance-ar-overdue-print-group-header">
                <strong>Cliente:</strong> {displayFinanceText(group.customerName)}
                {" · "}
                <strong>CNPJ:</strong> {displayFinanceText(group.customerDocument)}
                {" · "}
                <strong>Total vencido:</strong> {formatFinanceCurrency(group.totalOverdue)}
                {" · "}
                <strong>Títulos:</strong> {group.titlesCount}
                {" · "}
                <strong>Maior atraso:</strong> {formatFinanceInteger(group.maxDaysOverdue)} dias
              </div>
              <table className="finance-ar-overdue-print-detail-table">
                ...
                <tbody>
                  {group.titles.map((row) => (
                    <tr key={row.id}>
                      <td className="col-wrap">{displayFinanceText(row.customerName)}</td>
                      <td>{displayFinanceText(row.customerDocument)}</td>
                      <td>{displayFinanceText(row.documentNumber)}</td>
                      <td>{displayFinanceText(row.nfeNumber)}</td>
                      <td>{displayFinanceText(row.salesOrderNumber)}</td>
                      ...
                      <td className="col-wrap col-obs">
                        {truncateArOverduePrintText(row.description, 80)}
                      </td>
                    </tr>
                  ))}
                </tbody>
