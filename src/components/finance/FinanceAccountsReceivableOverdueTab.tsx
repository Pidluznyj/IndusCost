...
export function FinanceAccountsReceivableOverdueTab({
  ...
  return (
    <div className="space-y-4 finance-ar-overdue-root" ref={printRef}>
      <div className="finance-ar-overdue-screen-only flex flex-wrap items-start justify-between gap-3">
        <div>
      ...
      <div className="finance-ar-overdue-print-header hidden print:block">
        <h1 className="text-xl font-bold text-black">Relatório de Contas a Receber Atrasadas</h1>
        <p className="text-sm text-gray-700 mt-1">
          Gerado em {payload ? formatFinanceDateTime(payload.generatedAt) : "—"}
          {filterChips.length > 0 ? ` · ${filterChips.join(" · ")}` : ""}
        </p>
      </div>

      {error ? (
        <div className="rounded-lg border border-red-200 bg-red-50 px-3 py-2 text-sm text-red-800 finance-ar-overdue-screen-only">
          {error}
        </div>
      ) : null}

      {filterChips.length > 0 ? (
        <div className="flex flex-wrap gap-2 finance-ar-overdue-screen-only">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-3 finance-ar-overdue-filters finance-ar-overdue-screen-only">
        ...
        <>
          ...
          <section className="rounded-xl border border-[#E5E7EB] bg-white p-4">
            ...
            {payload && payload.pagination.totalPages > 1 ? (
              <div className="flex justify-end gap-2 mt-3 finance-ar-overdue-screen-only">
                <button
                  type="button"
          ...
          <footer className="hidden print:block text-xs text-gray-600 mt-6 pt-4 border-t">
            IndusCost · Relatório gerencial de Contas a Receber Atrasadas
          </footer>
        </>
      )}
    </div>
