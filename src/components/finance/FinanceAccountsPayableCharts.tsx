...
import type {
  FinanceApAgingBucket,
  FinanceApMonthlyDue,
  FinanceApPaymentSummary,
  FinanceApTopDebtor,
...
export function FinanceApTopDebtorsChart({ rows }: { rows: FinanceApTopDebtor[] }) {
  const data = (rows ?? []).slice(0, 10).map((r) => ({
    label: (r.personName ?? r.personCnpj ?? "Fornecedor").slice(0, 24),
  ...
  return (
    <ChartCard title="Top 10 fornecedors devedores" subtitle="Ranking por saldo em aberto" empty={empty}>
      <ResponsiveContainer width="100%" height="100%">
