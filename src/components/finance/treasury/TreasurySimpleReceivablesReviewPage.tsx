/**
 * Página — revisão simples de recebimentos do dia.
...
import {
  TREASURY_SIMPLE_RECEIVABLES_REVIEW_SUBTITLE,
...
function readFilters(params: URLSearchParams): TreasurySimpleReviewFilterState {
  ...
  const category = (
    categoryRaw === "ALL" ||
    (TREASURY_SIMPLE_RECEIVABLE_REVIEW_CATEGORIES as readonly string[]).includes(
      categoryRaw
    )
      ? categoryRaw
      : "ALL"
...
export function TreasurySimpleReceivablesReviewPage() {
  const auth = useAuth();
  ...
  const [detailRow, setDetailRow] = useState<TreasuryReceivableListItemDto | null>(
    null
  );
  ...
  useEffect(() => {
    ...
    const found = rows.find((r) => r.titleId === detailId) ?? null;
    setDetailRow(found);
  }, [detailId, rows]);
  ...
  const openDetails = (row: TreasuryReceivableListItemDto) => {
