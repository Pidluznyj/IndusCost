...
export const FINANCE_AR_TABLE_MAX_HEIGHT = "max-h-[min(70vh,640px)]";

export function FinanceArScrollableTable({
  children,
  className,
  tableClassName,
}: {
...
export function FinanceArStickyTableHead({ children }: { children: React.ReactNode }) {
  return (
...
export function FinanceArErrorBanner({
  message,
  onDismiss,
}: {
  message: string;
...
export function FinanceArSuccessBanner({
  message,
  onDismiss,
}: {
  message: string;
...
export function FinanceArTabNav({
  tabs,
  activeId,
  onChange,
}: {
  ...
  return (
    <nav
      ...
      aria-label="Abas de Contas a Receber"
    >
      {tabs.map((tab) => {
...
export function FinanceArLoadingBlock({ label }: { label: string }) {
  return (
