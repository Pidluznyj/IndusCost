...
import { financeArDataQualitySeverityLabel } from "@/src/lib/financeAccountsReceivableDataQuality";

function severityClass(severity: FinanceArDataQualityAlertItem["severity"]): string {
  switch (severity) {
    case "critical":
...
export function FinanceAccountsReceivableDataQualityPanel({
  alerts,
  onViewTitles,
}: {
          ...
          >
            <div className="flex items-start justify-between gap-2">
              ...
              <span className="text-[10px] font-bold uppercase shrink-0 opacity-80">
                {financeArDataQualitySeverityLabel(alert.severity)}
              </span>
            </div>
            ...
            {onViewTitles ? (
              <button
                type="button"
                onClick={() => onViewTitles(alert.key as FinanceArDataQualityAlertKey)}
