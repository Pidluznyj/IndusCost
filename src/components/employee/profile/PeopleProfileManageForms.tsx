...
import {
  ...
  HR_EMPLOYEE_BENEFIT_STATUS_LABELS,
...
export function BenefitsManageForm({
  ...
  // O benefício de um registro existente não muda: o catálogo só serve à inclusão.
  useEffect(() => {
    if (isEdit) return;
    let cancelled = false;
  ...
  const showAmount = canViewValues && Boolean(record ? record.isFinancial : selected?.isFinancial);

  const buildEditPayload = (): Record<string, unknown> => ({
    startDate,
    endDate: endDate || null,
    planName: trimOrNull(planName),
    status,
    notes: trimOrNull(notes),
    ...(showAmount ? { amount: amountOrNull(amount) } : {}),
  });
  const initialPayload = useInitialPayload(isEdit, buildEditPayload);

  return (
    <ProfileManageSection title={isEdit ? "Editar benefício" : "Registrar benefício"} inline={isEdit}>
      <form
        className="grid gap-3 max-w-xl"
        onSubmit={async (e) => {
          e.preventDefault();
          ...
          try {
            if (record) {
              const changed = await patchChangedFields(
                `/api/employees/${employeeId}/benefits/${record.id}`,
                initialPayload.current,
                buildEditPayload()
              );
              if (changed || !onCancel) onSaved();
            ...
            setError(
              err instanceof Error
                ...
                : isEdit
                  ...
                  : "Não foi possível registrar o benefício."
            );
          } finally {
            setSaving(false);
          }
      ...
      >
        {record ? (
          <ReadOnlyField
            label="Encargo ou benefício"
            value={record.typeLabel ? `${record.name} · ${record.typeLabel}` : record.name}
          />
        ) : (
        ...
        {isEdit ? (
          <Field label="Situação">
            ...
            >
              {optionsWithCurrent(HR_EMPLOYEE_BENEFIT_STATUSES, record?.status).map((s) => (
                <option key={s} value={s}>
                  {labelFrom(HR_EMPLOYEE_BENEFIT_STATUS_LABELS, s)}
                </option>
              ))}
            </select>
          </Field>
        ...
        <FormActions
          saving={saving}
          disabled={!isEdit && catalog.length === 0}
          label={isEdit ? "Salvar" : "Registrar benefício"}
          onCancel={isEdit ? onCancel : undefined}
        />
      </form>
    </ProfileManageSection>
