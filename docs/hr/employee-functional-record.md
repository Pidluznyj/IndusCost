...
- **2026-09-22** — Excluir **cargo** do cadastro operacional (Configurações → Estrutura Operacional → Cargos e Salários): botão só para **SUPER_ADMIN**, e `DELETE /api/roles/:id` passa a exigir SUPER_ADMIN. Cargo é obrigatório no colaborador (`Employee.roleId`) e no roteiro de produção (`ProductRouting.roleId`), sem cascata: só sai cargo **sem uso**; em uso, a tela e o servidor (409 `ROLE_IN_USE`) dizem onde trocar (`deleteRoleIfUnused`, `src/lib/settingsOperationalCatalog*.ts`). A listagem traz `employeeCount` / `routingCount` e o card mostra o uso. O histórico da ficha guarda o nome do cargo em texto e segue legível.
- **2026-09-22** — Excluir encargo/benefício do cadastro oficial (Configurações → Estrutura Operacional → Encargos e Benefícios): botão só para **SUPER_ADMIN**, e `DELETE /api/payroll-components/:id` passa a exigir SUPER_ADMIN (antes bastava `admin.settings.operational:manage`, sem tratamento de erro). A exclusão tira a verba de todos os colaboradores (cascata em `EmployeePayrollComponent`, muda a referência de custo) e grava `BENEFIT_CHANGE` "<verba> removido" no histórico de cada um (`deletePayrollComponentWithHistory`). A listagem traz `employeeCount` e o card mostra em quantos colaboradores a verba está marcada.
