...
| Linha do tempo mensal | `AR_MOVEMENT_TIMELINE_MONTHLY` | `buildExecutiveMonthlyTimeline` `dateAxis: "movement"` | idem + overlay | AR: movement (settlement + fev/2026) e aberto por due. AP: SEMPRE dueDate (`AP_MONTHLY_DUE_TIMELINE`) | gerencial AR / AP | year + identidade | recebido no eixo movement; AP pago (com caixa) e aberto por dueDate | tabela linha do tempo | aberto mensal do planejado (mesmo dueDate) | recebido mensal do planejado | Overlay e eixo de baixa (só AR) |
