...
Fonte: `src/lib/treasury/treasuryFeatureFlags.ts` — mestra **opt-in** (ausente = OFF); subflags ausentes = ON só com mestra ON; opt-out `0`/`false`; flag/valor desconhecidos **fail-closed**. Catálogo `TREASURY_FEATURE_FLAG_IDS`: **15** ids (1 mestra + 14 subflags). Ativação ADMIN: [ACTIVATION.md](./ACTIVATION.md).
