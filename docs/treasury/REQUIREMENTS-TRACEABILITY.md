...
**Atualizado:** 2026-07-28 (auditoria final Prompt **68**)  
...
4. Validação RC: `docs/treasury/RELEASE-CANDIDATE-VALIDATION.md`.
...
| R03 | Saldo observado/calculado/conciliado | Y | `treasuryFinancialPositionService.server.ts` + repos ledger/OFX | `GET …/accounts/:id/balance-position` + dashboard + report `daily-position` | `/finance/treasury` | `treasuryFinancialPosition*.test.ts`, `treasuryTraceabilityGapsApi.test.ts` | DONE | P68: ledger ACTIVE → calculado; OFX `ledgerBalance` → conciliado; sem inventar igualdade | Conciliado MISSING se OFX sem ledgerBalance |
...
| R12 | Projeção contratual/provável/confirmada | Y | projection engine/APIs | `POST …/projections/calculate`, compare/latest | `/finance/treasury/projections` (botão recalcular) | `treasuryProjection*.test.ts` | DONE | 3 cenários + FE calculate | — |
...
| R17 | Alertas | Y | alert rules + settings | `GET …/alerts`, `GET/PUT …/alert-settings` | dashboard/agenda + `/alert-settings` | `treasuryAlertRules.test.ts`, gaps wiring | DONE | feed + UI settings + audit UPDATE | sem push externo (fora de escopo atual) |
...
| R21 | Conciliação bancária | Y | match service + workspace + client API | `POST …/reconciliations` (accept), `…/unmatch`, `…/reverse`, `GET …/reconcile/workspace` | `/finance/treasury/reconcile` (+ bank-movements reverse) | reverse/match tests + gaps wiring | DONE | P68: FE accept/unmatch no workspace (além do reverse) | Match TITLE completo continua via API; UI workspace usa UNIDENTIFIED |
...
| R26 | Observabilidade | Y | availability+health | `GET …/availability`, `GET …/health` | indicadores freshness | availability + gaps wiring | DONE | P68: health com flag+ACL+probe schema (não ASSUMED_OK) | métricas OFX/closing ainda básicas |
| R27 | Testes | Y | `npm run test:treasury` | — | — | suite unit+integ+E2E | DONE | P59–P61 + gaps | 1 skip gated Postgres |
| R28 | Documentação | Y | `docs/treasury/*` + `manuals/*` | — | — | — | DONE | + `RELEASE-CANDIDATE-VALIDATION.md` (P68); README + 10–19 + manuais | PRD/TDD externos ausentes no repo |
...
**Commit Prompt 63:** `0b687bb`  
**Auditoria final Prompt 68:** ver `IMPLEMENTATION_STATUS.md` Prompt **68** + `RELEASE-CANDIDATE-VALIDATION.md`.
