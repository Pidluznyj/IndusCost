...
**Key files**: `employeeRegistration.ts`, `peopleProfile.server.ts`, `peopleProfileCapabilities.ts`, `peopleProfileAccess.ts`, `peopleProfileHierarchy.server.ts`, `peopleProfileMutations.server.ts` (create + PATCH/DELETE mutations of every ficha record, photo, history linkage), `peopleProfileRecordEdits.ts` (browser-safe PATCH parsing/validation, career type lock, photo magic-byte sniffing), `employeesPermissions.ts` (server permission bag + "Editor de RH" rule), `employeePersonalHr.ts` / `employeeAdminHr.ts` (cadastro validation + PII redaction, incl. estado civil / cidade / UF / CEP and `workSchedule`), `employeeHrUi.ts` (`EMPLOYEE_FICHA_TABS`, `EMPLOYEE_FICHA_RECORD_TABS`, `MARITAL_STATUS_OPTIONS`, `BRAZIL_UF_OPTIONS`), `src/components/employee/EmployeeEditRecordsPanel.tsx` (ficha records inside the "Editar colaborador" modal, mounted outside the cadastro `<form>`; records save immediately). Reference doc: `docs/hr/employee-functional-record.md`.
**Exports**: `loadPeopleProfileSummary`, `buildPeopleProfileCapabilities`, `canAccessEmployeeRecord`, `buildHrOrgChart`, `buildEmployeePermissionBag` (+ `EmployeeCanonicalAccessSnapshot`), `buildCanonicalAccessSnapshot` (`src/lib/security/requireResource.ts`), `applyCompensationAdjustment` (`historicalOnly`), `applyCareerMovement` (`recordOnly`), `update*`/`delete*` for `CompensationAdjustment`, `CareerEvent`, `EmployeeBenefit`, `EmployeeAbsence`, `EmergencyContact`, `EpiDelivery`, `EmployeeDocument`, `EmployeeNote`, `saveEmployeePhoto` / `removeEmployeePhoto` / `readEmployeePhotoFile`, `findLinkedHistoryEvent`, `parse*Patch`, `assertCareerEventEditable`, `resolveCareerEventTypeChange`, `sniffImageContentType`, `buildEmployeePhotoUrl`, `normalizeEmployeeState` / `normalizeEmployeeZipCode` / `formatZipCodeMask`, `normalizeEmployeeWorkSchedule`.
**Dependencies**: `employeesAccess.js`, Prisma.
**Dependents**: `src/components/employee/*` (dashboard, org chart, profile tabs, edit-records panel), `src/components/EmployeeModule.tsx`.
**Gotchas**: `employees.view` never grants compensation **values**, only events — a security-sensitive distinction. Cycle-detection in manager hierarchy has a duplicated "source of truth" (JS version tested; DB uses a recursive CTE) that must stay behaviorally in sync. History is append-only by default, but ficha record corrections (PATCH/DELETE) and retroactive "registro histórico" entries (`historicalOnly` adjustments, `recordOnly` career events) are **record-only**: they never touch `Employee.salary` or the Employee snapshot — do not "fix" them to re-sync the snapshot, and do not route current-state changes through them. Career PATCH/DELETE is type-locked (`PEOPLE_CAREER_EDITABLE_EVENT_TYPES`, else 409 `HISTORY_EVENT_LOCKED`; `COMPENSATION_ADJUSTMENT` rows are corrected via the compensation routes). Satellite records (benefit, absence, EPI delivery, document, note) link to their history row through `metadata { recordType, recordId }` with an exact legacy-match fallback that syncs/deletes at most one row. "Editor de RH" (legacy `employees.edit` OR canonical `admin.employees:update`) gets every ficha capability including R$ values unless there is an explicit individual deny on `admin.employees.compensation_values:view`; the rule lives only in `buildEmployeePermissionBag` (the pure `buildPeopleProfileCapabilities` invariants are unchanged), and `requireResourceOrHrEditor` in `server.ts` supplies only a `DENY_DEFAULT` on `admin.employees.*` facets for the lookup / person-link routes. Audit details for these routes carry ids/booleans/field names only — never amounts, CPF or phones.
